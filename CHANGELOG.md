...
- Valgrind memcheck docker variant (`t/docker/Dockerfile.valgrind` + `t/valgrind.sh`): per-request leak detection + invalid-read/write/uninitialised-value catch with a curated nginx suppression file. Exercises the compression hot path via `accept-encoding.sh` in single-process foreground mode (so the worker stays inside the valgrind trace). Note: the `dict-reload` reload-loop CDict-leak assertion is exclusive to plain `t/run.sh` — that script requires a master process and is incompatible with valgrind's foreground requirement. Per-request CDict alloc/free imbalances are still observable via the hot-path script.
